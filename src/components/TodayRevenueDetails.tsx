...
import { Search, X, Filter, IndianRupee } from 'lucide-react';
...
export function TodayRevenueDetails() {
  ...
  // Fetch vehicles checked out today (revenue generated)
  useEffect(() => {
  ...
  const totalRevenue = filteredVehicles.reduce((sum, v) => sum + (v.charge || 0), 0);
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="border-0 bg-white/80 backdrop-blur-lg shadow-2xl overflow-hidden relative before:absolute before:inset-0 before:bg-gradient-to-br before:from-white/60 before:to-white/30 before:pointer-events-none">
        <CardHeader className="pb-3 sm:pb-4 bg-gradient-to-r from-green-50/80 to-emerald-50/80 backdrop-blur-md rounded-t-xl border-b border-white/50 relative z-10">
          <CardTitle className="text-lg sm:text-xl text-slate-800">Today's Revenue Filters</CardTitle>
          <CardDescription className="text-xs sm:text-sm text-slate-600">
            Filter and analyze today's revenue transactions
          </CardDescription>
        </CardHeader>
      ...
      <Card className="border-0 bg-white/90 backdrop-blur-sm shadow-lg">
        <CardHeader className="pb-3 sm:pb-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-t-xl">
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-lg sm:text-xl">Today's Revenue Records</CardTitle>
              <CardDescription className="text-xs sm:text-sm mt-1">
                {filteredVehicles.length} transaction(s) today
              </CardDescription>
            </div>
        ...
        <CardContent className="pt-4">
          {loading ? (
            ...
            <div className="text-center py-12 text-slate-500">
              ...
              <p className="text-base">No revenue records found for today</p>
              {hasActiveFilters && (
