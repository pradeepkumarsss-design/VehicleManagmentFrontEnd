...
import { Search, Calendar, X, Filter, TrendingUp } from 'lucide-react';
...
export function RevenueDetails() {
  ...
  // Fetch all completed vehicles (revenue history)
  useEffect(() => {
  ...
  const filteredRevenueRecords = useMemo(() => {
    ...
    if (dateRange.from && dateRange.to) {
      ...
      filtered = filtered.filter(v => {
        ...
        const checkOutDate = new Date(v.checkOutTime);
      ...
      filtered = filtered.filter(v => {
        if (!v.checkOutTime) return false;
        return new Date(v.checkOutTime) >= fromDate;
      });
      ...
      filtered = filtered.filter(v => {
        if (!v.checkOutTime) return false;
        return new Date(v.checkOutTime) <= toDate;
      });
    }
    ...
    return filtered;
  }, [vehicles, searchQuery, vehicleTypeFilter, dateRange, sortBy]);

  const filteredRevenue = filteredRevenueRecords.reduce((sum, v) => sum + (v.charge || 0), 0);
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="border-0 bg-white/80 backdrop-blur-lg shadow-2xl overflow-hidden relative before:absolute before:inset-0 before:bg-gradient-to-br before:from-white/60 before:to-white/30 before:pointer-events-none">
        <CardHeader className="pb-3 sm:pb-4 bg-gradient-to-r from-purple-50/80 to-pink-50/80 backdrop-blur-md rounded-t-xl border-b border-white/50 relative z-10">
          <CardTitle className="text-lg sm:text-xl text-slate-800">Revenue Filters</CardTitle>
          <CardDescription className="text-xs sm:text-sm text-slate-600">
            Filter and analyze your revenue records
          </CardDescription>
        </CardHeader>
        <CardContent className="pt-4 relative z-10">
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2 items-center">
              <Select value={sortBy} onValueChange={(value: 'date' | 'charge' | 'duration') => setSortBy(value)}>
                <SelectTrigger className="w-48 h-10">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="date">Sort by Date (Latest)</SelectItem>
      ...
      <Card className="border-0 bg-white/90 backdrop-blur-sm shadow-lg">
        <CardHeader className="pb-3 sm:pb-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-t-xl">
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-lg sm:text-xl">Revenue Records</CardTitle>
              <CardDescription className="text-xs sm:text-sm mt-1">
                {filteredRevenueRecords.length} record(s) found
              </CardDescription>
            </div>
        ...
        <CardContent className="pt-4">
          {loading ? (
            ...
            <div className="text-center py-12 text-slate-500">
              ...
              <p className="text-base">No revenue records found</p>
              {hasActiveFilters && (
            ...
            <div className="rounded-lg border border-slate-200 overflow-hidden">
              <div className="overflow-x-auto">
                <Table>
                  ...
                  <TableBody>
                    {filteredRevenueRecords.map((vehicle) => (
